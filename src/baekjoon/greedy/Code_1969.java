        ...
        for(int i=0; i<cipher; ++i){        //  첫번째 자리부터 마지막 자리까지
            ...
            for(int j=0; j<number; ++j){    //  DNA 들을 하나씩 검사
                switch (dna[j].charAt(i)){  //  j번째 dna의 i번째 자릿수를 배열에 저장
                    case 'A' : ++nucle[0]; break;
                    case 'C' : ++nucle[1]; break;
                    case 'G' : ++nucle[2]; break;
                    case 'T' : ++nucle[3]; break;
                }
            }
            ...
            if(max==nucle[0]) {minDNA+="A";}
            else if(max==nucle[1]) {minDNA+="C";}
            else if(max==nucle[2]) {minDNA+="G";}
            else if(max==nucle[3]) {minDNA+="T";}
        }
